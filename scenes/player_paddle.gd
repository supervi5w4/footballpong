...
# Ограничения по X:
#   • Слева: фиксированный отступ LEFT_MARGIN_PX (по умолчанию 200 px)
#   • Справа: центр экрана минус center_bias_px (чуть не доходим до центра)
# ------------------------------------------------------------
...
@export var center_bias_px: int = 50   # на сколько пикселей левее центра ограничивать
...
func _clamp_x() -> void:
	var vp: Rect2 = get_viewport_rect()  # у CanvasItem; даёт прямоугольник вьюпорта
	var half := _resolve_half_size()

	var min_x := vp.position.x + float(LEFT_MARGIN_PX) + half.x

	var right_limit := vp.position.x + vp.size.x
	if use_center_as_right_limit:
		right_limit = vp.position.x + vp.size.x * 0.5 - float(center_bias_px)

	var max_x := right_limit - half.x

	# Подстраховка: если окно очень узкое и границы пересеклись
	if min_x > max_x:
		max_x = min_x
